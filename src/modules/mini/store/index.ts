...
export class MiniKancolleStore extends Store {
  ...
  @action
  public addNewUser(userId: string) {
  ...
  public getMyPublishedOrdersByUserId(userId: string) {
    return _(this.orders)
      .filter((order) => order.sellerId === userId)
      .value();
  }

  public getMyAcceptedOrdersByUserId(userId: string) {
    return _(this.orders)
  ...
  public getTradeRate(type: ResourceType) {
    ...
    const resourceTradeRate = _(resourceTotalAmount).map((r) => Number((r / resourceTotalAmount[type - 1]).toFixed(3)));
