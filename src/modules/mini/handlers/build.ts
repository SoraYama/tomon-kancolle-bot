...
import { showResource, computeExtraWeight, showShip, weightBalance, findConfigShipById } from '../utils';
...
const build = (resourceString: string[] = [], user: User, isNyk: boolean) => {
  try {
    ...
    logger.info(`建造结果 - ${user.id} [${inputResource.join(', ')}] ${selectedShip.name} ${isNyk} ${nyk.value}`);
