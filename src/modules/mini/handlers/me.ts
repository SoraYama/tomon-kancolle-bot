...
const me = (getMessage: (content: string) => string, user: User | null) => {
  if (!user) {
  ...
  const infoMap = {
    ...
    升级所需まるゆ数量: userLevelInfo.upgradeRequirement === null ? '已满级' : `${userLevelInfo.upgradeRequirement}个`,
    资源兑换手续费率: `${userLevelInfo.tradeTaxRate * 100}%`,
    舰队详情: ships,
  };
  return getMessage(_.map(infoMap, (v, k) => `${k}:\n${v}`).join('\n\n'));
};
